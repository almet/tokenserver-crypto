...
namespace crypto{
     ...
     **/
    byte* deriveKey(byte* ikm, byte* salt, int size, byte* info = NULL){

        byte* okm;

        // fill-in the result with zeroes
...
int main(int argc, const char *argv[])
{
    ...
    crypto::writeFile("master", ikm);

    byte* master = crypto::readFile("master");
    byte* salt = crypto::generateRandomSalt(78);
    byte* okm = crypto::deriveKey(master, salt, 78);
